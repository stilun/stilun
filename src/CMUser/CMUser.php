	...
	*/
	public function Init() 
	{
		try 
		{
			...
			$this->db->ExecuteQuery(self::SQL('insert into user'), array('root', 'The Administrator', 'stig@stilun.de', $password['algorithm'], $password['salt'], $password['password']));
	 ...
	 */
	 ...
	 {
	 	 $password = array(
	 	 	 'algorithm'=>($algorithm ? $algorithm : CStilun::Instance()->config['hashing_algorithm']),
	 	 	 'salt'=>null
	 	 	 );
	 ...
	 *
	 * @param $plain string the password plain text to use as base.
	 * @param $algorithm string the algorithm used to hash the user salt/password.
	 ...
	 */
	  ...
	  */
	  public function Create($acronym, $password, $name, $email) 
	  {
	  	  ...
	  	  $idUserGroup = 2;
	  	  $this->db->ExecuteQuery(self::SQL('insert into user2group'), array($idUser, $idUserGroup));
