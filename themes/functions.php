...
function get_debug() 
{
	...
	if(isset($ly->config['debug']['stilun']) && $ly->config['debug']['stilun']) 
	{
		$html .= "<hr><h3>Debuginformation</h3><p>The content of CStilun:</p><pre>" . htmlent(print_r($ly, true)) . "</pre>";
	}
	if(isset($ly->config['debug']['session']) && $ly->config['debug']['session']) 
	{
		$html .= "<hr><h3>SESSION</h3><p>The content of CStilun->session:</p><pre>" . htmlent(print_r($ly->session, true)) . "</pre>";
...
{
	$messages = CStilun::Instance()->session->GetMessages();
	$html = null;
	if(!empty($messages)) 
	{
...
{
	return 'http://www.gravatar.com/avatar/' . md5(strtolower(trim(CStilun::Instance()->user['email']))) . '.jpg?r=pg&amp;d=wavatar&amp;' . ($size ? "s=$size" : null);
}
...
{
	return CStilun::Instance()->request->base_url . trim($url, '/');
}
...
{
	return CStilun::Instance()->CreateUrl($urlOrController, $method, $arguments);
}
...
{
	return create_url(CStilun::Instance()->themeUrl . "/{$url}");
}
...
{
	return create_url(CStilun::Instance()->themeParentUrl . "/{$url}");
}
...
{
	return CStilun::Instance()->request->current_url;
}
...
{
	return CStilun::Instance()->views->Render($region);
}
...
{
	return CStilun::Instance()->views->RegionHasView(func_get_args());
}
